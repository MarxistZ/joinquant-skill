...
> Tick 级策略在每个 Tick 快照到达时执行一次，适合高频或短线交易。需要 Tick 权限（会员或积分兑换）。

## 前置条件

- **必须开启真实价格模式**：`set_option('use_real_price', True)`
- **`handle_data` 和 `run_daily(..., 'every_bar')` 不会在 Tick 策略中被调用**
...
**参数**：
- `context`：Context 对象，存放当前账户/标的持仓信息
- `tick`：Tick 对象，存放触发该次调用的 Tick 数据（含最新价、成交量、盘口等字段）

**返回值**：None

---

## subscribe — 订阅 Tick 事件
...
**参数**：
- `security`：标的代码（字符串）或代码列表（列表）。支持股票、期货、中证指数、场内基金。**不能直接订阅主力合约或期货指数代码**
...
def initialize(context):
    ...
    g.code1 = 'RB1909.XSGE'
    run_daily(before_market_open, time='0830',
              reference_security='RB9999.XSGE')
    run_daily(after_market_close, time='1530',
              reference_security='RB9999.XSGE')
...
- **市价单**：按最新价 + 滑点撮合（启用盘口撮合时按对手盘）
- **限价单**：每个 Tick 尝试撮合，委托价满足条件时以委托价成交
- **按 Tick 撮合时不检查成交量**：满足价格条件后剩余部分全部成交
...
- ❌ 在收盘函数中没有 `unsubscribe_all()` → 下一天可能残留订阅
